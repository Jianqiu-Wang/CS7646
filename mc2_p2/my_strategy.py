...
def plot_short_entries_exits(macd, signalline, df):
    ...
    short_entries = np.where(find_short_entries_exits == -1)
    short_entries_list = [i.tolist() for i in short_entries][0]
    short_exits = np.where(find_short_entries_exits == -1)
    ...
    while i < len(short_entries_list):
        ...
        trades_df.loc[trades_i] = [df.iloc[short_entries_list[i]].name, 'IBM', 'SELL', 50]
        trades_i += 1
        ...
        while j < len(short_exits_list):
            if short_exits_list[j] > current_entry:
                ...
                trades_df.loc[trades_i] = [df.iloc[short_exits_list[j]].name, 'IBM', 'BUY', 50]
                trades_i += 1
...
def plot_long_entries_exits(macd, signalline, df):
    ...
    find_long_entries_exits = np.pad(np.diff(np.array(macd > signalline).astype(int)),
                               (1, 0), 'constant', constant_values=(0, ))
    long_entries = np.where(find_long_entries_exits == 1)
    long_exits = np.where(find_long_entries_exits == -1)
    ...
    while i < len(long_entries_list):
        ...
        while j < len(long_exits_list):
            if long_exits_list[j] > current_entry:
                ...
                trades_df.loc[trades_i] = [df.iloc[long_exits_list[j]].name, 'IBM', 'SELL', 94]
                trades_i += 1
